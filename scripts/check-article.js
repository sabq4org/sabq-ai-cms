...
async function checkArticle(slugOrId) {
  ...
  try {
    ...
    if (article) {
      ...
      const effectiveType = article.content_type || 
        (article.article_type === 'news' ? 'NEWS' : 'OPINION');
      console.log(`نوع المحتوى الفعلي: ${effectiveType}`);
      
      if (effectiveType === 'NEWS') {
        console.log(`📰 الرابط الصحيح: https://sabq.io/news/${article.slug || article.id}`);
      } else {
        console.log(`📝 الرابط الصحيح: https://sabq.io/article/${article.slug || article.id}`);
      }
