...
async function fixArabicUrls() {
  console.log(
    "🚨 بدء إصلاح مشكلة الروابط العربية المسببة لخطأ React #130...\n"
  );
  ...
  try {
    ...
    const arabicSlugs = articles.filter(
      (article) => article.slug && /[\u0600-\u06FF]/.test(article.slug)
    );
    ...
    const report = {
      ...
      summary: {
        problem:
          "Arabic slugs in URLs were causing React #130 errors in production",
    ...
    const stillArabic = allRemainingSlugs.filter(
      (article) => article.slug && /[\u0600-\u06FF]/.test(article.slug)
    );
    ...
    console.log(
      `✅ مُعدل النجاح: ${(
        ((arabicSlugs.length - stillArabic.length) / arabicSlugs.length) *
        100
      ).toFixed(1)}%`
    );
