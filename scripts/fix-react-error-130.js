...
function fixReactImports(filePath) {
  ...
  if (importLines.length !== uniqueImports.length) {
    ...
    const firstNonImportLine = content
      .split("\n")
      .findIndex(
        (line) =>
...
console.log("\n🔍 فحص التعارضات...");
try {
  const result = execSync("npm ls react react-dom", {
    encoding: "utf8",
    stdio: "pipe",
  });
...
console.log(
  "2. إذا استمر الخطأ، شغل: node scripts/fix-react-error-130.js --clean-modules"
);
