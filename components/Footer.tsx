...
import { Facebook, Twitter, Instagram, Youtube, Mail, Heart, Globe } from 'lucide-react';
...
export default function Footer() {
  ...
  return (
    <footer className="bg-gray-50 dark:bg-gray-900 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 dark:border-gray-800 mt-20">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="md:col-span-2 lg:col-span-1">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 leading-relaxed">
            ...
            <div className="mt-6 flex gap-3">
              {socialLinks.map((social) => (
                <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="p-2 bg-gray-100 dark:bg-gray-800 dark:bg-gray-800 rounded-full text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:bg-blue-100 hover:text-blue-600 dark:hover:bg-blue-600 dark:hover:text-white transition-colors">
                  {social.icon}
                </a>
              ))}
            </div>
          ...
          {footerSections.map((section) => (
            <div key={section.title}>
              <h3 className="font-bold text-gray-800 dark:text-gray-100 dark:text-white mb-4">{section.title}</h3>
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link.url}>
                    <Link href={link.url} className="text-gray-600 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-500 transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-16 border-t border-gray-200 dark:border-gray-700 dark:border-gray-700 pt-8 flex flex-col md:flex-row items-center justify-between">
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500">
           ...
           <div className="flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500">
