...
import { 
  Newspaper, Tag, Calendar, TrendingUp, TrendingDown, Minus, 
  Users, Eye, Activity, Clock, Zap, Award, Globe
} from 'lucide-react';
...
const TrendIndicator = memo(({ 
  ...
  return (
    <div className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full ${trendData.bgColor}`}>
      {trendData.icon}
...
const StatItem = memo(({
  ...
  const content = (
    <div className="flex items-center gap-2 group hover:scale-105 transition-all duration-200">
      {/* الأيقونة */}
      <div className={`p-1.5 rounded-xl ${bgColor} group-hover:scale-110 transition-transform duration-200`}>
        <Icon className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} ${color}`} />
      </div>

      {/* المحتوى */}
      <div className="flex-1 min-w-0">
        <p className={`${compact ? 'text-xs' : 'text-xs'} ${
        ...
        <div className="flex items-center gap-2">
          <p className={`${compact ? 'text-sm' : 'text-base'} font-bold ${
            ...
            {loading ? (
              <span className={`inline-block ${compact ? 'w-8 h-3' : 'w-12 h-4'} bg-gray-300 dark:bg-gray-600 rounded animate-pulse`} />
            ) : (
              value
            )}
          </p>
          ...
          {/* مؤشر الاتجاه */}
          {!loading && showTrend && trend && percentage !== undefined && trend !== 'stable' && (
            <TrendIndicator 
              trend={trend} 
...
const MobileStatsBar = memo(({
  ...
  const { darkMode } = useDarkModeContext();
  const { stats, loading, error, lastFetch } = useStatsData(autoRefresh, refreshInterval);
  ...
  return (
    <div className={containerClasses}>
      ...
      {/* الإحصائيات الرئيسية */}
      <div className={`grid ${
        variant === 'compact' ? 'grid-cols-3' :
        variant === 'minimal' ? 'grid-cols-2' :
...
// مكونات متخصصة
export const CompactMobileStatsBar = memo((props: Omit<MobileStatsBarProps, 'variant'>) => (
  <MobileStatsBar {...props} variant="compact" />
));

export const MinimalMobileStatsBar = memo((props: Omit<MobileStatsBarProps, 'variant'>) => (
  <MobileStatsBar {...props} variant="minimal" showTrends={false} showHealth={false} />
));
