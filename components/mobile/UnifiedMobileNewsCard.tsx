...
export default function UnifiedMobileNewsCard({
  ...
  if (variant === "smart-block") {
    return (
      <Link href={getArticleLink(data)} className={`group block ${className}`}>
        <div
            ...
            <div className="mb-1 flex flex-col gap-0.5">
              <div className="flex items-center gap-2">
                ...
                >
                  <span className="text-xs">
                    {data.breaking ? "🔥" : data.featured ? "⭐" : "📰"}
                  </span>
                  {data.breaking ? "عاجل" : data.featured ? "مميز" : "خبر"}
                </span>
              </div>
              ...
              <div className={`mt-1 ${darkMode ? "text-blue-300" : "text-blue-600"}`}>
                <p className="text-[10px] font-medium">
                  {data.category_name ? `أحدث أخبار ${data.category_name}` : "خبر جديد"}
                </p>
              </div>
            </div>
