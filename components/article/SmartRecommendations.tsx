...
import { 
  Brain, TrendingUp, Clock, Eye, ThumbsUp, 
  Sparkles, Target, Users, ArrowRight, Star,
...
export default function SmartRecommendations({ currentArticleId, recommendations }: SmartRecommendationsProps) {
  const { darkMode } = useDarkModeContext();
  const [activeTab, setActiveTab] = useState<'highly_recommended' | 'trending_now' | 'reading_pattern'>('highly_recommended');
  ...
  const RecommendationCard = ({ article, type }: { article: any; type: string }) => {
    const matchType = article.ai_match_type ? matchTypeConfig[article.ai_match_type] : null;
    ...
    `}>
      ...
      `}>
        ...
        <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          مقالات مختارة بعناية بواسطة الذكاء الاصطناعي حسب اهتماماتك وأسلوب قراءتك
        </p>
      </div>
