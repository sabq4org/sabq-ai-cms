...
const FeaturedNewsCard = memo(({
  ...
  const imageUrl = article.featured_image || article.image_url || article.image || "";
  ...
  if (variant === "hero") {
    return (
      <Link href={getArticleLink(article)} className={`block w-full ${className}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              {article.category && (
                ...
                >
                  {article.category.icon && <span className="text-xs">{article.category.icon}</span>}
                  {article.category.name}
                </span>
              )}
            </div>
...
export const HeroNewsCard = memo((props: Omit<FeaturedNewsCardProps, 'variant'>) => 
  <FeaturedNewsCard {...props} variant="hero" />
);

export const CompactNewsCard = memo((props: Omit<FeaturedNewsCardProps, 'variant'>) => 
