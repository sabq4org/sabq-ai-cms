...
import type {
  ...
  PerformanceAlert,
  PerformanceSnapshot,
} from "@/lib/modules/performance-optimization/types";
import {
  Activity,
  AlertTriangle,
  BarChart3,
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      const [
        ...
        fetch("/api/performance-optimization?action=current_performance")
          .then((r) => r.json())
          .catch((e) => ({ success: false, error: e.message })),
        fetch("/api/performance-optimization?action=alerts")
          .then((r) => r.json())
          .catch((e) => ({ success: false, error: e.message })),
        fetch("/api/performance-optimization?action=recommendations")
          .then((r) => r.json())
          .catch((e) => ({ success: false, error: e.message })),
        fetch("/api/performance-optimization?action=results")
          .then((r) => r.json())
          .catch((e) => ({ success: false, error: e.message })),
        fetch("/api/performance-optimization?action=rules")
          .then((r) => r.json())
          .catch((e) => ({ success: false, error: e.message })),
      ]);

      if (performance.success && performance.data)
        setCurrentPerformance(performance.data);
      ...
      if (resultsData.success && resultsData.data)
        setOptimizationResults(
          Array.isArray(resultsData.data) ? resultsData.data : []
        );
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {currentPerformance.overall_score.toFixed(1)}
                  </p>
                </div>
                <BarChart3 className="h-8 w-8 text-blue-600" />
              </div>
              <Progress
                value={currentPerformance.overall_score}
                className="mt-2"
              />
            </CardContent>
          </Card>
                ...
                {currentPerformance?.system_info && (
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Progress
                        value={currentPerformance.system_info.cpu_usage}
                      />
                    </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-medium text-green-600">
                            +{rec.estimated_impact.response_time_improvement}%
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-500">توفير الذاكرة</p>
                          <p className="font-medium text-blue-600">
                            +{rec.estimated_impact.memory_savings}%
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-500">توفير المعالج</p>
                          <p className="font-medium text-purple-600">
                            +{rec.estimated_impact.cpu_savings}%
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-500">توفير النطاق</p>
                          <p className="font-medium text-orange-600">
                            +{rec.estimated_impact.bandwidth_savings}%
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-lg font-medium">
                  لم يتم تنفيذ أي تحسينات بعد
                </p>
                <p className="text-gray-600">
                  ابدأ بتفعيل المراقبة لرؤية النتائج
                </p>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      {result.completed_at && (
                        <p className="text-sm text-gray-600">
                          انتهى في:{" "}
                          {result.completed_at.toLocaleString("ar-SA")}
                        </p>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
          ...
          {!rules || rules.length === 0 ? (
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-lg font-medium">لا توجد قواعد تحسين محددة</p>
                <p className="text-gray-600">
                  سيتم إنشاء القواعد تلقائياً عند بدء المراقبة
                </p>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">
                        نوع التحسين: {rule.type} | عدد الإجراءات:{" "}
                        {rule.actions.length} | آخر تحديث:{" "}
                        {rule.updated_at.toLocaleDateString("ar-SA")}
                      </p>
                    </div>
