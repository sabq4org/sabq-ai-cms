...
import { 
  Eye, Clock, Calendar, Volume2, Play, Pause, 
  Star, TrendingUp, ChevronLeft, ChevronRight,
  Quote, User, MessageCircle, ThumbsUp, Share2,
  Bookmark, Tag, ArrowRight, Lightbulb,
...
const OpinionCard = ({ article, isHero = false, darkMode = false }: { article: OpinionArticle; isHero?: boolean; darkMode?: boolean }) => {
      ...
      <Card className={`group overflow-hidden transition-all duration-300 hover:shadow-xl transform hover:-translate-y-1 ${
        ...
        <CardContent className="p-0">
          ...
          <div className={`p-6 ${isHero ? 'pb-6' : 'h-full flex flex-col'}`}>
            ...
            {/* العنوان */}
            <h3 className={`font-bold mb-3 leading-tight transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400 ${
              isHero 
                ? 'text-xl md:text-2xl' 
                : 'text-lg'
            ...
            <div className="flex items-center justify-between mt-auto">
              ...
              <div className="flex items-center gap-2">
                {/* تاريخ النشر */}
                <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  {format(new Date(article.publishedAt), 'dd MMM', { locale: ar })}
                </span>
...
const AuthorCard = ({ author, darkMode = false }: { author: OpinionAuthor; darkMode?: boolean }) => (
  <Link href={`/opinion/author/${author.id}`}>
    <Card className={`group transition-all duration-300 hover:shadow-lg transform hover:-translate-y-1 ${
        ...
        <div className="flex justify-center gap-4 text-xs">
          ...
          <div className={darkMode ? 'text-gray-300' : 'text-gray-700'}>
            <div className="font-bold">
              {author.totalViews ? (author.totalViews > 1000 ? `${(author.totalViews / 1000).toFixed(1)}k` : author.totalViews) : 0}
            </div>
            <div>مشاهدة</div>
          </div>
        </div>
...
export default function EnhancedTodayOpinionsSection({ darkMode = false }: TodayOpinionsSectionProps) {
  ...
  const fetchOpinionData = async () => {
    try {
      ...
      const transformedArticles: OpinionArticle[] = articles.map((article: any) => ({
        id: article.id,
        title: article.title,
      ...
      // تقسيم المقالات
      const featured = transformedArticles.filter(article => article.isFeatured).slice(0, 3)
      const regular = transformedArticles.filter(article => !article.isFeatured).slice(0, 8)
      ...
      <section className={`py-12 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="animate-pulse space-y-8">
            <div className={`h-8 rounded ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`} />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[...Array(3)].map((_, i) => (
                <div key={i} className={`h-64 rounded ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`} />
              ))}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => (
                <div key={i} className={`h-48 rounded ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`} />
              ))}
            </div>
          </div>
        </div>
      ...
      <section className={`py-12 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center">
            <Quote className={`w-16 h-16 mx-auto mb-4 opacity-50 ${isDark ? 'text-gray-600' : 'text-gray-400'}`} />
            ...
            <Button onClick={fetchOpinionData}>
              إعادة المحاولة
            </Button>
          </div>
        </div>
          ...
          <div className="mb-12">
            ...
            >
              {featuredAuthors.map((author) => (
                <div key={author.id} className="flex-shrink-0 w-48" style={{ scrollSnapAlign: 'start' }}>
