...
class ImageMigrationTool {
      ...
      const categories = await this.prisma.category.findMany({
        where: {
          OR: [
            ...
            { metadata: { path: ['cover_image'], string_contains: 'amazonaws.com' } }
          ]
        }
      });

      console.log(`📋 تم العثور على ${categories.length} تصنيف يحتوي على صور S3\n`);
  ...
  async migrateCategoryImage(category) {
    ...
    const isComplexS3Url = s3ImageUrl.includes('X-Amz-') || 
    ...
    try {
      ...
      const cloudinaryUrl = await this.uploadToCloudinary(s3ImageUrl, `categories/${category.slug}`);
      
      if (cloudinaryUrl) {
        await this.updateCategoryImage(category, cloudinaryUrl);
        console.log(`   ✅ تم الترحيل بنجاح: ${cloudinaryUrl.substring(0, 60)}...`);
        this.migratedCount++;
      } else {
  ...
  async replaceWithDefaultImage(category) {
    const defaultImages = {
      'محليات': 'https://images.unsplash.com/photo-1449824913935-59a10b8d2000?auto=format&fit=crop&w=800&q=80',
      'العالم': 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=800&q=80',
    ...
    const defaultImage = defaultImages[category.name] || 
  ...
  printSummary() {
    ...
    console.log(`🎯 المجموع: ${this.migratedCount + this.skippedCount + this.errorCount}\n`);
