...
export const ENHANCED_CACHE_KEYS = {
  ...
  CATEGORY_ARTICLES: (slug: string, params: string) =>
    `category:${slug}:articles:${params}`,
...
export function generateCacheKey(
  ...
  const paramString =
    Object.keys(sortedParams).length > 0
      ? `:${Buffer.from(JSON.stringify(sortedParams))
          .toString("base64")
          .substring(0, 20)}`
      : "";
  ...
  try {
    await Promise.all(
      keysToDelete.map((pattern) =>
        pattern.includes("*") ? redis.clearPattern(pattern) : redis.del(pattern)
      )
    );
  } catch (error) {
