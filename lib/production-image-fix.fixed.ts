...
// تحسين: إضافة الروابط المطلقة للصور الافتراضية
export const PLACEHOLDER_IMAGES = {
  article: process.env.NODE_ENV === 'production' 
    ? `${PRODUCTION_DOMAIN}/images/placeholder-news.svg`
    : `/images/placeholder-news.svg`,
  analysis: process.env.NODE_ENV === 'production' 
    ? `${PRODUCTION_DOMAIN}/images/deep-analysis-default.svg`
...
export function getProductionImageUrl(
  ...
  if (
    ...
    console.log(`🖼️ استخدام صورة الافتراضية: ${fallbackType}`);
  ...
  // تشخيص إضافي: تسجيل معلومات الصورة
  console.log(`🔍 معالجة صورة: ${imageUrl.substring(0, 50)}...`);
  ...
  if (imageUrl.startsWith("/")) {
    // تحسين: استخدام window.location.origin إذا كنا في بيئة المتصفح
    if (typeof window !== "undefined") {
      return `${window.location.origin}${imageUrl}`;
    }
    // وإلا استخدم الدومين المعرف
  ...
  if (
    ...
    try {
      ...
      // تحسين: تسجيل الخطأ بشكل أفضل
      console.error(`❌ فشل معالجة رابط S3: ${imageUrl}`, error);
  ...
  if (imageUrl.startsWith("http://") || imageUrl.startsWith("https://")) {
    ...
    try {
      ...
      console.error(`❌ رابط غير صالح: ${imageUrl}`, error);
      // رابط غير صالح
      return PLACEHOLDER_IMAGES[fallbackType];
    }
  }
