...
export default function UnifiedNewsCreatePageUltraEnhanced() {
  ...
  const [aiTriggerTimeout, setAiTriggerTimeout] =
  ...
  const calculateCompletion = useCallback(() => {
    ...
    const checks = [
      ...
      {
        field: formData.keywords.length > 0,
        weight: 5,
  ...
  const generateAutoSuggestions = useCallback(
    async (content: string, forceGenerate = false) => {
      // التحقق من صحة المحتوى
      if (!content || typeof content !== "string") {
        console.warn(
          "generateAutoSuggestions: المحتوى المرسل غير صالح:",
          typeof content
        );
        return;
      }
      ...
      try {
        ...
        if (titleResponse.status === "fulfilled" && titleResponse.value.ok) {
          ...
          if (titleData.result) {
            ...
            const titles = titleData.result
              ...
              .filter(
                (title: string) => title.length > 10 && title.length < 100
              )
              .slice(0, 3);
        ...
        if (
          excerptResponse.status === "fulfilled" &&
          excerptResponse.value.ok
        ) {
        ...
        if (
          keywordsResponse.status === "fulfilled" &&
          keywordsResponse.value.ok
        ) {
  ...
  const applySuggestion = useCallback(
    (type: "title" | "excerpt" | "keywords", value: any, index?: number) => {
      switch (type) {
        case "title":
          if (typeof value === "string") {
            ...
            typeof index === "number" &&
            aiAutoSuggestions.titleSuggestions[index]
          ) {
            setFormData((prev) => ({
              ...prev,
              title: aiAutoSuggestions.titleSuggestions[index],
            }));
            toast.success("تم تطبيق العنوان المقترح");
          }
          break;
        ...
        case "excerpt":
          setFormData((prev) => ({
            ...prev,
            excerpt: aiAutoSuggestions.excerptSuggestion,
          }));
    ...
    const loadInitialData = async () => {
      try {
        ...
        console.log("📡 استجابات API:", {
          ...
          reporters: {
            status: reportersResponse.status,
            ok: reportersResponse.ok,
          },
        });
        ...
        if (categoriesResponse.ok) {
          ...
          console.log(
            `📂 تم جلب ${loadedCategories.length} تصنيف`,
            loadedCategories
          );
          ...
          if (loadedCategories.length > 0) {
            ...
            console.log(
              `🎯 تصنيف افتراضي: ${
                loadedCategories[0].name_ar || loadedCategories[0].name
              } (${defaultCategoryId})`
            );
          }
        } else {
        ...
        if (reportersResponse.ok) {
          ...
          if (authorsData.success && authorsData.authors) {
            ...
            console.log(
              `👥 تم جلب ${convertedReporters.length} مؤلف من article_authors`,
              convertedReporters
            );
            ...
            if (convertedReporters.length > 0) {
              ...
              console.log(
                `👤 مؤلف افتراضي: ${convertedReporters[0].name} (${defaultReporterId})`
              );
            }
          } else {
        ...
        if (isEditMode && articleId) {
          ...
          try {
            const articleResponse = await fetch(
              `/api/articles/${articleId}?all=true`
            );
            ...
            if (articleResponse.ok) {
              ...
              if (article) {
                ...
                setFormData({
                  ...
                  seoTitle:
                    article.metadata?.seo_title || article.seo_title || "",
                  seoDescription:
  ...
  const handleSave = async (status: "draft" | "published") => {
    ...
    try {
      ...
      const articleData: any = {
        ...
        seo_keywords:
          formData.keywords.length > 0 ? formData.keywords.join(", ") : null,
      ...
      if (!articleData.category_id) {
        console.error(
          "❌ خطأ: لا يوجد معرف تصنيف! المقال لن يُنشر بدون تصنيف."
        );
  ...
  const generateFromContent = async () => {
    ...
    try {
      ...
      console.log("🔍 بدء استخراج المحتوى:", {
        ...
        editorRefMethods: editorRef.current
      ...
      if (editorRef.current) {
        try {
          console.log("🔍 طرق المحرر المتاحة:", {
            ...
            editorMethods: editorRef.current.editor
              ? Object.keys(editorRef.current.editor)
              : [],
          });
          ...
          if (editorRef.current.editor?.getText) {
            ...
            if (content && typeof content === "object") {
              if (content.html) {
                ...
                console.log(
                  "✅ استخدام getContent.html:",
                  rawExtractedText?.length
                );
              } else if (content.text) {
                rawExtractedText = content.text;
                console.log(
                  "✅ استخدام getContent.text:",
                  rawExtractedText?.length
                );
              } else {
          ...
          console.log("🔍 النص المستخرج من المحرر:", {
            ...
            preview:
              typeof rawExtractedText === "string"
                ? rawExtractedText.substring(0, 100)
                : String(rawExtractedText || "").substring(0, 100),
          });
      ...
      toast.error(
        error instanceof Error ? error.message : "حدث خطأ في التوليد التلقائي"
      );
    } finally {
      setIsAILoading(false);
    }
      ...
      <Button
        onClick={() => {
          ...
          if (completionScore < 60) {
            toast.error(
              `المقال غير مكتمل بما يكفي للنشر (${completionScore}%). يرجى إكمال البيانات المطلوبة.`
            );
            return;
          }
