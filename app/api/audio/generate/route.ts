      ...
      try {
        const archiveResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/audio/archive`, {
          ...
          body: JSON.stringify({
            ...
            duration: Math.ceil(enhancedText.length / 15) + ' ثانية',
            voice: voice,
            text_length: enhancedText.length,
            is_daily: false, // إضافة علامة للنشرة اليومية
            is_published: false // النشرة اليومية تكون منشورة تلقائياً
          })
        });
      ...
      if (error.response?.status === 404 && error.response?.data?.detail?.status === 'voice_not_found') {
        ...
        try {
          ...
          const archiveData = {
            ...
            text_preview: enhancedText.substring(0, 200) + '...',
            created_at: new Date().toISOString(),
            is_daily: false, // النشرة اليومية تكون منشورة تلقائياً
            is_published: false,
            fallback_used: true
          };
