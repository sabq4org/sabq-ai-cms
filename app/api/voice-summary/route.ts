...
/**
 * دالة رفع الملفات الصوتية للخادم السحابي
 * @param audioData - بيانات الملف الصوتي
...
async function uploadAudioToCloud(
  ...
  // يمكن تطوير هذه الدالة لاحقاً لدعم Cloudinary أو AWS S3
  // في الوقت الحالي نرجع Base64 كخطة بديلة
  const timestamp = Date.now();
  const safeFileName = fileName
    .replace(/[^a-zA-Z0-9\u0600-\u06FF\s]/g, "")
    .substring(0, 50);

  // محاكاة رفع للخادم السحابي
  if (process.env.CLOUDINARY_CLOUD_NAME && process.env.CLOUDINARY_API_KEY) {
    // TODO: تطبيق Cloudinary API هنا
    console.log("📁 محاولة رفع للـ Cloudinary:", safeFileName);
    // في الوقت الحالي نرجع Base64
    return `data:audio/mpeg;base64,${audioData.toString("base64")}`;
  }

  if (process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY) {
    // TODO: تطبيق AWS S3 API هنا
    console.log("📁 محاولة رفع لـ AWS S3:", safeFileName);
    // في الوقت الحالي نرجع Base64
    return `data:audio/mpeg;base64,${audioData.toString("base64")}`;
  }

  // إذا لم تكن هناك إعدادات خادم سحابي، ارجع Base64
  return `data:audio/mpeg;base64,${audioData.toString("base64")}`;
}
...
function selectVoiceForContent(content: string, category?: string): string {
  ...
  if (
    category?.includes("رياض") ||
    contentLower.includes("مباراة") ||
    contentLower.includes("فريق")
  ) {
  ...
  if (
    category?.includes("ثقاف") ||
    category?.includes("مجتمع") ||
    contentLower.includes("فن")
  ) {
  ...
  if (
    category?.includes("تقني") ||
    category?.includes("اقتصاد") ||
    contentLower.includes("شرك")
  ) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!article.excerpt) {
      return NextResponse.json(
        { error: "المقال لا يحتوي على موجز" },
        { status: 400 }
      );
    }
    ...
    if (voiceType === "auto") {
      selectedVoice = selectVoiceForContent(
        article.excerpt,
    ...
    console.log(
      `🔊 استخدام الصوت: ${selectedVoice} للفئة: ${
        article.categories?.name || "غير محدد"
      }`
    );
    ...
    console.log(
      `📝 النص المحسن للقراءة: ${optimizedText.substring(0, 100)}...`
    );
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        return NextResponse.json({
          ...
          voiceInfo:
            VOICE_OPTIONS[
              Object.keys(VOICE_OPTIONS).find(
                (key) =>
                  VOICE_OPTIONS[key as keyof typeof VOICE_OPTIONS].id ===
                  selectedVoice
              ) as keyof typeof VOICE_OPTIONS
            ],
          message:
            "صوت تجريبي محسن - سيتم استبداله بصوت حقيقي عند إصلاح مفتاح ElevenLabs",
        });
      } else {
        return NextResponse.json(
          {
    ...
    if (process.env.UPLOAD_AUDIO_TO_CLOUD === "true") {
      // رفع للخادم السحابي (Cloudinary, AWS S3, etc.)
      try {
        // محاولة رفع الملف للخادم السحابي
        const cloudUploadUrl = await uploadAudioToCloud(
          audioData,
          articleTitle
        );
        ...
        console.warn(
          "⚠️ فشل رفع الملف الصوتي للخادم السحابي، سيتم الحفظ محلياً:",
          cloudError
        );
        // العودة للحفظ المحلي كخطة بديلة
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "test_voice") {
      const {
        voiceId,
        text = "هذا اختبار للصوت. مرحباً بكم في صحيفة سبق الإلكترونية.",
      } = await request.json();
      ...
      if (!apiKey) {
        return NextResponse.json(
          { error: "مفتاح ElevenLabs غير متوفر" },
          { status: 500 }
        );
      }
