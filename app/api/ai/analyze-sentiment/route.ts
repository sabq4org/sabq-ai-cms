...
function analyzeSentimentBasic(content: string, title: string): "neutral" | "positive" | "critical" {
  const text = `${title} ${content}`.toLowerCase();
  
  // كلمات إيجابية
  const positiveWords = [
    "ممتاز", "رائع", "مذهل", "نجاح", "تطور", "تحسن", "إنجاز", "فخر",
    "سعادة", "أمل", "تقدم", "ازدهار", "نمو", "إبداع", "ابتكار", "تميز",
    "انتصار", "إنجاز", "تحقيق", "استثمار", "فرصة", "مستقبل", "حلم",
    "excellent", "great", "amazing", "success", "progress", "achievement"
  ];
  
  // كلمات نقدية
  const criticalWords = [
    "مشكلة", "خطأ", "فشل", "أزمة", "تراجع", "انخفاض", "سوء", "ضعف",
    "قلق", "خوف", "تحدي", "صعوبة", "عقبة", "مخاطر", "تهديد", "انتقاد",
    "اعتراض", "رفض", "استياء", "غضب", "احتجاج", "مقاومة", "معارضة",
    "problem", "error", "failure", "crisis", "decline", "criticism"
  ];
  
  // كلمات تحليلية تشير للنقد البناء
  const analyticalWords = [
    "تحليل", "دراسة", "بحث", "تقييم", "مراجعة", "فحص", "اختبار",
    "analysis", "study", "research", "evaluation", "review", "examination"
  ];
...
async function analyzeSentimentAI(content: string, title: string): Promise<{
  ...
  const words = content.split(/\s+/).filter(word => 
    word.length > 3 && 
    !['على', 'في', 'من', 'إلى', 'عن', 'مع', 'هذا', 'هذه', 'التي', 'الذي'].includes(word)
  );
