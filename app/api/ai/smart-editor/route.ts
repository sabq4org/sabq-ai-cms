...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // تحميل برومبت التحرير الافتراضي من الإعدادات (إن وجد)
...
`.trim();
    ...
    async function loadAISettings() {
      try {
        ...
        const aiTemp: number = typeof data.default_temperature === 'number' ? data.default_temperature : 0.85;
        return { aiPrompt: aiPrompt || DEFAULT_EDITOR_PROMPT, aiTemp };
      } catch {
        return { aiPrompt: DEFAULT_EDITOR_PROMPT, aiTemp: 0.85 };
      }
    }
...
قيود إخراج تقنية (JSON فقط دون أي شرح):
{
  "title": "<عنوان قصير غني بالمعلومة والكلمات المفتاحية، بلا أفعال تقريرية>",
  "subtitle": "<عنوان فرعي مكمّل 40–80 حرفاً>",
  "smart_summary": "<موجز ذكي 250–400 حرفاً>",
  "keywords": ["<كلمة/عبارة>", "..."],  // 5–8 عناصر، أسماء وكيانات وموضوعات فقط
...
التدقيق قبل الإخراج:
- احسب طول smart_summary. إن كان <250 زد التفصيل، وإن كان >400 فاختصره بدون فقدان المعلومة.
- تأكد أن keywords لا تتضمن أفعالاً من القائمة الممنوعة.
- لا تُرجع سوى JSON صالح. لا تعليقات ولا أسطر زائدة بعد القوس الأخير.
`.trim();
