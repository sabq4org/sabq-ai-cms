...
export async function GET(
  ...
  try {
    ...
    const stats = {
      ...
      avgViewsPerArticle: reporter.total_articles > 0 
        ? Math.round((reporter.total_views || 0) / reporter.total_articles)
        : 0,
      engagementRate: reporter.total_views > 0
        ? Math.round(((reporter.total_likes || 0) + (reporter.total_shares || 0)) / (reporter.total_views || 1) * 100 * 100) / 100
        : 0,
      // إحصائيات شهرية (تقديرية)
      monthlyArticles: Math.round((reporter.total_articles || 0) / 12 * 100) / 100,
