...
/**
 * جلب بيانات الولاء للمستخدم من قاعدة البيانات الحقيقية
 */
const getUserLoyaltyData = async (userId: number) => {
  try {
    ...
    const totalPoints = userPoints.reduce(
      (sum, point) => sum + point.points_earned,
      0
    );

    // حساب نقاط الشهر الحالي
    ...
    const pointsThisWeek = userPoints
      .filter((point) => new Date(point.created_at) >= currentWeek)
      .reduce((sum, point) => sum + point.points_earned, 0);
...
const checkArticleLimit = async (
  ...
  try {
    ...
    console.error("خطأ في فحص حد المقال:", error);
    return false;
  }
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "READ" && duration) {
      if (duration >= 60) {
        rule = LOYALTY_RULES.find(
          (r) => r.action === "READ_long" && r.is_active
        );
      } else if (duration >= 30) {
    ...
    if (
      sourceId &&
      !checkArticleLimit(userId, action, sourceId, rule.max_per_article)
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    if (action === "read" && sessionId) {
      ...
      if (currentStreak === 5) {
        const bonusRule = LOYALTY_RULES.find(
          (r) => r.action === "bonus_streak"
        );
        if (
          bonusRule &&
          checkDailyLimit(userId, "bonus_streak", bonusRule.max_per_day)
        ) {
          const bonusPoints = {
            id: Date.now() + 1,
            action: "bonus_streak",
            ...
            metadata: {
              consecutive_articles: currentStreak,
          ...
          userData.summary.level = updateUserLevel(
            userData.summary.total_points
          );
          bonusAwarded = true;
        }
      }
    ...
    return NextResponse.json({
      ...
      message: `تم منح ${rule.points} نقطة بنجاح! ${
        bonusAwarded ? "+ 10 نقاط مكافأة!" : ""
      }`,
      point_details: {
        action,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pointsToNext =
      userData.summary.level === "Bronze"
        ? 100 - userData.summary.total_points
        : userData.summary.level === "Silver"
        ? 500 - userData.summary.total_points
        : userData.summary.level === "Gold"
        ? 1000 - userData.summary.total_points
        : 0;
    ...
    return NextResponse.json({
      ...
      level_progress: {
        ...
        progress_percentage:
          userData.summary.level === "Platinum"
            ? 100
            : Math.min(
                100,
                (userData.summary.total_points /
