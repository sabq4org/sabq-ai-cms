  ...
  try {
    ...
    try {
      ...
      if (articleAuthors.length > 0) {
        ...
        console.log(
          "⚠️ لا يوجد مؤلفين في article_authors، سيتم الجلب من team_members"
        );
    ...
    if (authors.length > 0) {
      console.log(
        `📋 أسماء المؤلفين: ${authors.map((a) => a.full_name).join(", ")}`
      );
    }
    ...
    return NextResponse.json({
      ...
      source:
        authors.length > 0 && authors[0].specializations !== undefined
          ? "article_authors"
          : "team_members",
    });
  } catch (error: any) {
