  ...
  try {
    ...
    if (!corner_id || !title || !content) {
      return NextResponse.json(
        {
          success: false,
          error: "البيانات الأساسية مطلوبة (الزاوية، العنوان، المحتوى)",
        },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < 5; i++) {
      const exists =
        await prisma.$queryRaw`SELECT id FROM muqtarab_articles WHERE slug = ${shortSlug};`;
