...
export async function GET(request: NextRequest) {
  try {
    ...
    const analysis = {
      ...
      articlesWithoutCategory: recentArticles.filter(a => !a.category_id).length,
      articlesWithoutAuthor: recentArticles.filter(a => !a.author_id && !a.article_author_id).length,
      categoryIssues: recentArticles.filter(a => a.category_id && !a.categories).length,
      authorIssues: recentArticles.filter(a => a.author_id && !a.author && !a.article_author).length
    };
    ...
    return NextResponse.json({
      ...
      sampleData: {
        recentArticles: recentArticles.map(a => ({
          ...
          author_name: a.author?.name || a.article_author?.full_name || 'غير مُرتبط',
