...
function NewspaperHomePage({ stats, initialArticles = [], initialCategories = [] }: NewspaperHomePageProps & { initialArticles?: any[], initialCategories?: any[] }): React.ReactElement {
  ...
  // التصنيفات - استخدام البيانات المبدئية
  ...
  // =============================
  // جلب التصنيفات عند التحميل - فقط إذا لم تكن موجودة
  useEffect(() => {
    if (initialCategories.length > 0) {
      return; // البيانات موجودة من الخادم
    }
  ...
  // =============================
  // جلب المقالات الأحدث - فقط إذا لم تكن موجودة
  useEffect(() => {
...
export default function ClientHomePage({ initialArticles = [], initialCategories = [], initialStats }: ClientHomePageProps) {
  ...
  // جلب الإحصائيات الحقيقية - فقط إذا لم تكن موجودة
  useEffect(() => {
    if (initialStats && !initialStats.loading) {
      return; // البيانات موجودة من الخادم
    }
  ...
  return <NewspaperHomePage stats={stats} initialArticles={initialArticles} initialCategories={initialCategories} />;
}