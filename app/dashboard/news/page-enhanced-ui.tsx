...
export default function NewsManagementPageEnhanced() {
    ...
    const fetchNewsData = async () => {
      try {
        ...
        const response = await fetch(
          "/api/articles?status=all&limit=100&sort=created_at&order=desc",
          {
        ...
        const mapped: NewsItem[] = result.articles.map((a: any) => {
          ...
          return {
            ...
            category_color:
              a.categories?.color || a.category_color || "#3B82F6",
            publishTime:
              a.published_at || a.created_at || new Date().toISOString(),
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (result.success) {
        setNewsData((prev) =>
          prev.map((n) =>
            n.id === id ? { ...n, status: "deleted" as NewsStatus } : n
          )
        );
    ...
    try {
      ...
      if (result.success) {
        setNewsData((prev) =>
          prev.map((n) =>
            n.id === articleId
              ? { ...n, isBreaking: !currentBreakingStatus }
              : n
          )
        );

        toast.success(
          currentBreakingStatus
            ? "تم إلغاء الخبر العاجل"
      ...
      toast.error(
        e instanceof Error ? e.message : "فشل تحديث حالة الخبر العاجل"
      );
  ...
  const handleCopy = (slugOrId: string) => {
    navigator.clipboard
      .writeText(`https://sabq.org/articles/${slugOrId}`)
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      if (result.success) {
        setNewsData((prev) =>
          prev.map((n) =>
            n.id === id ? { ...n, status: "draft" as NewsStatus } : n
          )
        );
  ...
  const stats = {
    ...
    avgRating:
      newsData.length > 0
        ? Math.round(
            (newsData.reduce((acc, n) => acc + n.rating, 0) / newsData.length) *
              10
          ) / 10
        : 0,
    breaking: newsData.filter((n) => n.isBreaking && n.status !== "deleted")
      .length,
  ...
  const filteredData = newsData
    .filter((item) => {
      ...
      if (activeTab === "breaking")
        return item.isBreaking && item.status !== "deleted";
    ...
    .filter((item) => {
      ...
      const searchInAuthor = item.author_name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase());
      const searchInCategory = item.category_name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase());
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-2">
                            ...
                            <div>
                              <Badge
                                className={`${
                                  getStatusBadge(news.status).color
                                } text-xs`}
                              >
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm">
                            ...
                            >
                              {new Date(news.publishTime).toLocaleTimeString(
                                "ar-SA",
                                {
                                  hour: "2-digit",
                                  minute: "2-digit",
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      handleStatusUpdate(news.id, "draft")
                                    }
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                {news.status !== "published" && (
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleStatusUpdate(news.id, "published")
                                    }
                                  >
