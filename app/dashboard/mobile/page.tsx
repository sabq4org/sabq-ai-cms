...
import EnhancedMobileDashboard, { StatCard } from '@/components/mobile/EnhancedMobileDashboard';
      ...
      `}>
        <div className="text-center">
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            تم تصميم هذه الواجهة خصيصاً للهواتف المحمولة لتوفير تجربة سلسة ومريحة
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard
            ...
            change={stats.todayArticles > 0 ? `+${stats.todayArticles} اليوم` : undefined}
            changeType="positive"
            icon={FileText}
          />
          <StatCard
            ...
            change={stats.weeklyGrowth > 0 ? `+${stats.weeklyGrowth}% هذا الأسبوع` : undefined}
            changeType="positive"
            icon={Eye}
          />
          <StatCard
          ...
          {loading ? (
            ...
            Array.from({ length: 3 }).map((_, i) => (
              <div
                ...
                `}
              >
                <div className={`w-8 h-8 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-300'}`} />
                <div className="flex-1 space-y-2">
                  <div className={`h-4 rounded ${darkMode ? 'bg-gray-700' : 'bg-gray-300'}`} />
                  <div className={`h-3 w-2/3 rounded ${darkMode ? 'bg-gray-700' : 'bg-gray-300'}`} />
                </div>
              </div>
            ))
...
function QuickActionCard({ title, subtitle, icon: Icon, href, color, darkMode }: QuickActionCardProps) {
  const colorMap = {
    ...
    gray: { bg: 'bg-gray-100 dark:bg-gray-700', icon: 'text-gray-600 dark:text-gray-300' },
