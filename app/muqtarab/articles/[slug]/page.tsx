...
export default function ArticleDetailPage() {
  ...
  // جلب بيانات المقال والمقالات ذات الصلة
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (articleData.article.angle?.id) {
          const relatedResponse = await fetch(
            `/api/muqtarab/angles/${articleData.article.angle.id}/articles?limit=6&exclude=${articleData.article.id}`
          );
          ...
          const crossAngleResponse = await fetch(
            `/api/muqtarab/cross-angle-recommendations?currentAngleId=${articleData.article.angle.id}&currentArticleId=${articleData.article.id}&limit=3`
          );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* شريط التنقل العلوي */}
      {article.angle && (
        <StickyHeader angle={article.angle} article={article} />
      )}
      ...
      <div className="max-w-4xl mx-auto px-3 md:px-4 py-4 md:py-8">
        ...
        <div className="hidden md:block">
          {article.angle && (
            <Breadcrumbs angle={article.angle} article={article} />
          )}
        </div>
        ...
        {/* تحليل AI للمقال */}
        {article.angle && (
          <AIAnalysisSection article={article} angle={article.angle} />
        )}
            ...
            <Link href={`/muqtarab/${article.angle.slug}`}>
              <Button
                ...
                style={{
                  ...
                  borderColor: article.angle.themeColor,
                }}
              >
                <ArrowLeft className="w-4 h-4 ml-2" />
                <span className="hidden sm:inline">
                  العودة إلى زاوية {article.angle.title}
                </span>
...
function StickyHeader({
  ...
  angle: { id: string; title: string; slug: string; themeColor: string };
  article: Article;
}) {
...
function ArticleHeader({ article }: { article: Article }) {
  return (
    <div className="mb-4 md:mb-8">
        ...
        {article.angle && (
          <Link href={`/muqtarab/${article.angle.slug}`}>
            <Badge
              ...
              style={{
                backgroundColor: article.angle.themeColor + "20",
                color: article.angle.themeColor,
              }}
            >
      ...
      {/* اقتباس افتتاحي ذكي */}
      {article.angle && (
        <OpeningQuote article={article} angle={article.angle} />
      )}
