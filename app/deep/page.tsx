...
export default function DeepAnalysisPage() {
  ...
  const [sortBy, setSortBy] = useState<"newest" | "views" | "reading_time">(
    "newest"
  );
    ...
    try {
      ...
      const totalViews = allArticles.reduce(
        (sum: number, article: Article) =>
          sum + (article.views || article.views_count || 0),
        0
      );
      const totalReadingTime = allArticles.reduce(
        (sum: number, article: Article) => sum + (article.reading_time || 10),
        0
      );
      ...
      setStats({
        ...
        avgReadingTime:
          allArticles.length > 0
            ? Math.round(totalReadingTime / allArticles.length)
            : 0,
      });
    } catch (error) {
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-700 dark:text-gray-300 font-medium">
                  {loading && page === 1
                    ...
                    : articles.length > 0
                    ? selectedCategory
                      ? `${articles.length} تحليل في ${getCategoryName(
                          selectedCategory
                        )}`
              ...
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  onChange={(e) => {
                    setSortBy(
                      e.target.value as "newest" | "views" | "reading_time"
                    );
                    setPage(1);
                  }}
          ...
          {loading && page === 1 ? (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-12 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                {selectedCategory
                  ? `لا توجد تحليلات في قسم ${getCategoryName(
                      selectedCategory
                    )}`
            ...
            <>
              ...
              <div
                className={
                  viewMode === "grid"
                    ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 content-start"
                    : "space-y-4"
                }
              >
                {articles.map((article) => (
                  <ArticleCard
                    ...
                    article={{
                      ...
                      featured:
                        article.featured || article.is_featured || false,
                      breaking:
                        article.breaking || article.is_breaking || false,
                    }}
                    viewMode={viewMode}
                  />
                ))}
