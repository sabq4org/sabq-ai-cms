    ...
    const fetchPersonalizedContent = async () => {
      ...
      try {
        ...
        const [allContent, breaking, later, recommended] = await Promise.all([
          fetch(
            `/api/content/personalized?user_id=${userId}&limit=20&type=all`
          ).then((res) => res.json()),
          fetch(
            `/api/content/personalized?user_id=${userId}&limit=5&type=breaking`
          ).then((res) => res.json()),
          fetch(
            `/api/content/personalized?user_id=${userId}&limit=5&type=read-later`
          ).then((res) => res.json()),
          fetch(
            `/api/content/personalized?user_id=${userId}&limit=10&type=recommended`
          ).then((res) => res.json()),
        ]);
      ...
      return (
        ...
        >
          <div className="p-6">
            ...
            <p
              className={`text-base mb-4 line-clamp-3 transition-colors duration-300 ${
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3
                className={`font-semibold mb-1 line-clamp-2 transition-colors duration-300 ${
                  darkMode ? "text-white" : "text-gray-900"
                }`}
              >
                {article.title}
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {personalizedContent && personalizedContent.articles.length > 0 && (
          <section className="mb-12">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 content-start">
        ...
        {(!personalizedContent ||
          personalizedContent.articles.length === 0) && (
          <div
