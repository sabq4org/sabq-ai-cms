...
export default function MuqtaribPage() {
  ...
  useEffect(() => {
    const fetchAngles = async () => {
      try {
        console.log("🔍 جاري جلب زوايا مُقترب...");
        ...
        if (response.ok) {
          ...
          console.error("❌ فشل في جلب الزوايا:", response.status);
  ...
  useEffect(() => {
    ...
    switch (selectedFilter) {
      ...
      case "trending":
        ...
        filtered = filtered.sort((a, b) => (b.articlesCount || 0) - (a.articlesCount || 0));
        break;
    }
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-12">
            <path d="M0,0V60c0,0,200,50,600,0s600,50,600,0V0" className="fill-current text-blue-50"></path>
          </svg>
        </div>
      </div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-3xl font-bold text-gray-900">
              {selectedFilter === "all" ? "جميع الزوايا" : 
          ...
          {filteredAngles.length === 0 ? (
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">لا توجد زوايا</h3>
    ...
    <Card className="group rounded-2xl overflow-hidden border-0 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      <div className="relative h-48 w-full overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <h3 className="text-white font-bold text-lg line-clamp-2">{angle.title}</h3>
        </div>
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <Link href={`/muqtarib/${angle.slug}`}>
          <Button variant="ghost" className="w-full justify-start text-blue-600 hover:text-blue-700 hover:bg-blue-50 p-0 h-8">
