...
import { HeroCard } from "@/components/muqtarib/HeroCard";
import {
  BookOpen,
  Calendar,
  Eye,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // جلب الزوايا
        const anglesResponse = await fetch("/api/muqtarib/angles", {
          cache: "no-store",
          headers: {
        ...
        // جلب المقال المميز
        try {
          const heroResponse = await fetch("/api/muqtarib/hero-article", {
            cache: "no-store",
            headers: {
              "Cache-Control": "no-cache",
            },
